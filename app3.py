...
# --- Minimal CSS (Only for Headers/Layout) ---
# Removed text area styling to restore default Streamlit UI
st.markdown("""
<style>
    .main-header {
        font-size: 2.5rem;
    ...
    /* Comparison box styling for outputs only */
    .comparison-box {
        padding: 15px;
        border-radius: 10px;
...
with st.sidebar:
    ...
    available_models = ["gpt-4o", "gpt-4-turbo", "gpt-3.5-turbo", "gpt-5-mini"] 
...
# --- Main App ---
st.title("🧪 Prompt Architect & Arena")
...
with tab1:
    ...
    with col1:
        ...
        # Use session state for value to allow updates from "Auto-Enhance"
        input_value = st.session_state.optimized_prompt if st.session_state.optimized_prompt else default_text
        
        # STANDARD STREAMLIT TEXT AREA (No Custom CSS)
        user_prompt = st.text_area("Enter your prompt here:", value=input_value, height=200)
    ...
    if run_pressed and user_prompt:
        with st.spinner("Generating output..."):
            ...
            st.subheader("Output")
            st.info(output) # Using standard st.info for visibility
        ...
        if not prompt_a or not prompt_b:
            ...
            with c1:
                with st.spinner("Running Side A..."):
                    ...
                    st.success(res_a) # Use standard success box
            ...
            with c2:
                with st.spinner("Running Side B..."):
                    ...
                    st.warning(res_b) # Use standard warning box (different color)
